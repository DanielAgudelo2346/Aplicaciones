...
{
    public partial class Eliminarad : MetroFramework.Forms.MetroForm
    {
        string usuario;
        string correo;
        string nombre;
        ...
        private void rellenar()
        {
            ...
            while (dr.Read())
            {
                lugar = dr["CasaDeEvento"].ToString();
                string[] partes = lugar.Split('-');
                //for (int i = 0; i < partes.Length; i++)
                //{
                //string element = partes[i];
                ...
                var consulta = "select * from tbeventos where Lugar ='" + partes[0] + "'Or Lugar='" + partes[1] + "'Or Lugar='" + partes[2] + "'Or Lugar='" + partes[3] + "'Or Lugar='" + partes[4] + "';";
                var c = new SqlConnection(@"Data Source=DESKTOP-5OEO849\SQLEXPRESS;Initial Catalog=ElectivaProg;Integrated Security=True");
                ...
                this.metroGrid1.Columns["Usuario"].Visible = false;
        ...
        private void Eliminarad_Load(object sender, EventArgs e)
        {
            SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-5OEO849\SQLEXPRESS;Initial Catalog=ElectivaProg;Integrated Security=True");
            SqlCommand cmd = new SqlCommand("select * from tblogin where Usuario='" + usuario + "'", conn);
            conn.Open();
        ...
        private void metroButton1_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.Yes)
            {
                ...
                con.Open();
                SqlCommand consulta = new SqlCommand("SELECT * FROM tblogin1 WHERE Usuario='"+usucliente+"'", con);
        ...
        private void metroGrid1_CurrentCellChanged(object sender, EventArgs e)
        {
            if (metroGrid1.CurrentRow != null && metroGrid1.CurrentRow.Index > -1)
            {
                ...
                usucliente = metroGrid1.CurrentRow.Cells[8].Value != null ? metroGrid1.CurrentRow.Cells[8].Value.ToString() : "";
        ...
        {
            Menuadmin b = new Menuadmin(usuario);
            b.Show();
            this.Hide();
        }
    }
